    ...
    public String[] solution(int totalTicket, String[] logs){
        ...
        for (String sentence : logs){
            ...
            if(command.equals("request")){
                ...
                if(!session.isTicketingSuccessTime(currentTime)){continue;} // 애는 그냥 넘겨도 됨
                if(answer_list.contains(name)) continue;
                totalTicket--;
                answer_list.add(session.getName());
                session.update(name, command, currentTime);
            }
            if(command.equals("leave")){
                if(!session.isTicketingSuccessTime(currentTime)){ session.update(name, command, currentTime); session.clear(); continue;}
                if(answer_list.contains(name)) {session.clear(); session.update(name, command, currentTime); continue;}
                totalTicket--;
                answer_list.add(session.getName());
                session.update(name, command, currentTime);
                session.clear();
...
class Session {
    ...
    public boolean isTicketingSuccessTime(LocalTime other){
